    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                // Allow access to a potential future login endpoint without authentication
                 .requestMatchers("/api/auth/**").permitAll() 
                 // Require authentication for all other requests
                .anyRequest().authenticated()
            )
            // Enable basic HTTP authentication as a starting point
            // We will likely replace this with form login or JWT later for the React frontend
